...
if (isset($_GET['userName'])) {
    $company = $_GET['userName'];

    // Use $userName as needed in your workerreviews.php file
    // ...
}
   ...
   if (isset($_POST['insert'])) {
    ...
    $sql = "INSERT INTO reviews (userName, company, description, rating) 
            VALUES ('$user', '$company', '$description', '$rating')";
...
if ($resultCompanies) {
    $companies = $resultCompanies->fetch_all(MYSQLI_ASSOC);
} else {
    // Handle the case where the query fails
    echo "Error fetching company names: " . $db->error;
}
?>
...
<body>
    ...
    <label for="user">Rated by:</label>
<input type="text" id="user" name="user" value="<?php echo $user; ?>" disabled>
        
<label for="company">User Rated :</label>
<input type="text" id="company" name="company" value="<?php echo $company; ?>" disabled>
            </select>
